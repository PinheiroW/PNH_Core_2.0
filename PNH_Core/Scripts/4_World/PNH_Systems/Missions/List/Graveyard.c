class GraveyardMission extends PNH_MissionBase
{
    ...
    void GraveyardMission()
    {
        ...
        m_MissionMessage1 = m_Config.Lore.MensagensRadio[0].Replace("Major General", "Major General " + m_SurvivorName);
    ...
    override void PlayerChecks(PlayerBase player)
    {
        ...
        {
            m_InnerWarned = true;
            EnviarAviso(m_MissionInformant, "Você chegou ao local. Procure por terra remexida perto dos túmulos e comece a cavar.");
        }

        // 3. GATILHO DE VITÓRIA: FINALIZAR DESENTERRAR
        if (!m_IsVictory && m_Stash && !m_Stash.IsUnderground())
        {
            m_IsVictory = true;
    ...
    override bool DeployMission()
    {
        ...
        if (m_Stash)
        {
            ...
            if (m_Chest && m_Config.Recompensas.Loadouts.Count() > 0)
            {
        ...
        PNH_Logger.Log("Missões", "[PNH_CORE] MISSÃO_INICIADA: Graveyard em " + m_MissionLocation + " | Coordenadas: " + m_MissionPosition.ToString());
        return true;
    }
}