class GanjaMission extends SurvivorMissions
{
	...
	{		
		...
		m_MissionMessage1 = SurvivorName +", um cara que trabalhava para mim me disse que cruzou uma variedade especial de Cannabis de várias plantas 'Cannabis ruderalis'. Só preciso disso para meus experimentos com os infectados. Ele disse que fertilizou as plantas-mãe com infectados fragmentados.";
		m_MissionMessage2 = "Acho que ele se referia a qualquer tipo de bebida fermentada com água, cinzas infectadas e pedaços de carne feitos em uma batedeira de cozinha. O impacto no crescimento e florescimento das plantas foi enorme. Também parece que contém uma substância diferente dos canabinóides nativos. Quero extrair essa substância para ver como ela age nos infectados.";
		m_MissionMessage3 = SurvivorName +" escondeu suas ervas em uma garagem em\n** "+ m_MissionLocation +" **\nEle certamente trancou as portas da garagem, então tente usar uma fechadura ou atire nelas, se não tiver nenhuma.";
	...
	{		
		...
		{			
			...
			MissionObject.GetInventory().CreateInInventory("Shovel");
			MissionObject.GetInventory().CreateInInventory("Canteen");	
	...
	void ObjDespawn() 
	{	
		//Despawn todos os objetos da missão no tempo limite da missão, exceto aqueles retidos até a próxima missão
		for ( int i = 0; i < m_MissionObjects.Count(); i++ )
		{
			//Exceção: MissionCar permanecerá, mas será danificado
	...
	{	//When player enters mission target zone at primary mission
		...
		m_MissionMessage1 = "Tudo bem, então você encontrou o carro. "+ SurvivorName +" provavelmente drenou todo o combustível e óleo, então prepare o carro antes dessa longa viagem. Verifique se a Cannabis seca está em algum lugar nas garagens e pegue a sacola e coloque todas as 25 peças nela.";
		m_MissionMessage2 = "Dirija para o  ** "+ m_MissionDescription[3] +" Posto de gasolina **\ne traga o Ganjabag para o caixa. "+ SurvivorName +"'s ganja tem a maior prioridade. Por favor, não roube ou fume sozinho porque pode ser muito perigoso.";
		m_MissionMessage3 = "Tenho algumas coisas para você no posto de gasolina. Tenha cuidado, pode haver bandidos no caminho para "+ m_MissionDescription[3] +". Eles poderiam interceptar nossa pequena conversa de rádio aqui. Boa sorte e dirija com cuidado!";
